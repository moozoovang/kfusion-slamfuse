...
slambench::values::PointCloudValue *Kfusion::getMap() {
	slambench::values::PointCloudValue *point_cloud = new slambench::values::PointCloudValue();

	short2 * volume_data = (short2*) malloc(volumeResolution.x * volumeResolution.y * volumeResolution.z
						* sizeof(short2));
		clEnqueueReadBuffer(commandQueue, ocl_volume_data, CL_TRUE, 0,
				volumeResolution.x * volumeResolution.y * volumeResolution.z
						* sizeof(short2), volume_data, 0, NULL, NULL);
	...
	Volume volume ;
	volume.size = volumeResolution;
	volume.dim  = volumeDimensions;
    ...
    for(unsigned int z=0; z<volume.size.z-1; z++)
        ...
        for(unsigned int y=0; y<volume.size.y-1; y++)
      {
        for (unsigned int x=0; x<volume.size.x-1; x++)
          {
            //Loop over all cubes
            const uint8_t cubeIndex = getCubeIndex(x,y,z,volume);
            ...
            for(int i=0; i<5; i++) {
				...
				auto tri1 = calcPtInterpolate((tri + 3*i)[0],x, y, z, volume);
				auto tri2 = calcPtInterpolate((tri + 3*i)[1],x, y, z, volume);
				auto tri3 = calcPtInterpolate((tri + 3*i)[2],x, y, z, volume);

				point_cloud->AddPoint(slambench::values::Point3DF(tri1.x, tri1.y, tri1.z));
				point_cloud->AddPoint(slambench::values::Point3DF(tri2.x, tri2.y, tri2.z));
				point_cloud->AddPoint(slambench::values::Point3DF(tri3.x, tri3.y, tri3.z));
...
slambench::values::PointCloudValue *Kfusion::getRaycast() {

	sb_float3 * ptr = (sb_float3*)malloc ( sizeof(sb_float3) * computationSize.x * computationSize.y ) ;

	clEnqueueReadBuffer(commandQueue, ocl_vertex, CL_TRUE, 0, sizeof(sb_float3) * computationSize.x * computationSize.y, ptr, 0, NULL, NULL);

	slambench::values::PointCloudValue *pointcloud = new slambench::values::PointCloudValue();

	for (unsigned int i = 0 ; i < computationSize.x * computationSize.y ; i ++)  {
		pointcloud->AddPoint({ptr[i].x, ptr[i].y, ptr[i].z});
	}
...
bool Kfusion::tracking(float4 k, float icp_threshold, uint tracking_rate,
	...
	for (int level = iterations.size() - 1; level >= 0; --level) {
		...
		for (int i = 0; i < iterations[level]; ++i) {
			...
			for (unsigned int j = 1; j < number_of_groups; ++j) {
				values[0] += values[j];
			}
