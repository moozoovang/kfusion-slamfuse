...
bool updatePoseKernel(Matrix4 & pose, const float * output,
		float icp_threshold, TooN::Vector<6> &x ) {
	bool res = false;
	TICK();
...
bool Kfusion::tracking(float4 k, float icp_threshold, uint tracking_rate,
	...
	for (int level = iterations.size() - 1; level >= 0; --level) {
		...
		for (int i = 0; i < iterations[level]; ++i) {
			...
			if (updatePoseKernel(pose, reductionoutput, icp_threshold, x))
				break;
...
slambench::values::PointCloudValue *Kfusion::getRaycast() {
	float3 *ptr = vertex;
	float3 *end = ptr + (computationSize.x * computationSize.y);
	...
	while(ptr < end) {
		pointcloud->AddPoint({ptr->x, ptr->y, ptr->z});
		ptr++;
	}
