...
bool Kfusion::tracking(float4 sk, float icp_threshold, uint tracking_rate, uint frame) {
...
slambench::values::PointCloudValue *Kfusion::getRaycast() {
	...
	for (float3 vert : vertices) {
		point_cloud->AddPoint(slambench::values::Point3DF(vert.x, vert.y, vert.z));
	}
